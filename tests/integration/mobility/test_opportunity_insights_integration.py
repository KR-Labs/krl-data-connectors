...
def ri_data(connector):
    ...
    """
    return connector.fetch_opportunity_atlas(
        geography='tract',
        state='44'
    )
...
class TestRealDataDownload:
    ...
    def test_download_tract_data(self, connector):
        ...
        data = connector.fetch_opportunity_atlas(
            geography='tract',
            state='44'
        )
...
class TestMultiStateQueries:
    ...
    def test_different_geographies_same_state(self, connector):
        ...
        state_data = connector.fetch_opportunity_atlas(
            geography='state',
            state=state
        )
...
class TestCompleteWorkflows:
    ...
    def test_county_level_analysis_workflow(self, connector):
        ...
        high_opp_tracts = county_data[
            county_data['kfr_pooled_p25'] > mean_mobility + std_mobility
        ]
        
        low_opp_tracts = county_data[
            county_data['kfr_pooled_p25'] < mean_mobility - std_mobility
        ]
...
class TestPerformance:
    ...
    def test_filtered_query_faster_than_full(self, connector):
        ...
        # Full state query
        start1 = time.time()
        full_data = connector.fetch_opportunity_atlas(
            geography='tract',
            state='44'
        )
        time1 = time.time() - start1
