...
common_metrics = st.sampled_from([
    ...
    'kfr_black_p25', 'kfr_white_p25', 'kfr_hisp_p25',
...
class TestStateFilteringProperties:
    ...
    @given(
        state_filter=valid_state_fips,
        other_state=valid_state_fips
    )
        ...
        with patch.object(connector, '_download_file') as mock_download:
            ...
            with patch('pandas.read_stata') as mock_read:
                ...
                if len(result) > 0 and 'state' in result.columns:
                    assert all(result['state'] == state_filter), \
    ...
    @given(state_code=valid_state_fips)
    @settings(max_examples=30, suppress_health_check=[HealthCheck.function_scoped_fixture])
    def test_state_filter_idempotent(
        self,
        connector,
        state_code
    ):
...
class TestMetricFilteringProperties:
    ...
    @given(
        metrics=st.lists(common_metrics, min_size=1, max_size=3, unique=True)
    )
    @settings(max_examples=30, suppress_health_check=[HealthCheck.function_scoped_fixture])
    def test_metric_filter_includes_only_requested_metrics(
        self,
        connector,
        metrics
    ):
        ...
        with patch.object(connector, '_download_file') as mock_download:
            ...
            with patch('pandas.read_stata') as mock_read:
                ...
                result_metrics = [col for col in result.columns if col not in ['state', 'county', 'tract', 'cz', 'czname']]
                ...
                for col in result_metrics:
                    ...
                    assert col in metrics or '_se' in col or 'count' in col, \
    ...
    @settings(max_examples=30, suppress_health_check=[HealthCheck.function_scoped_fixture])
    def test_single_metric_filter_reduces_columns(
        self,
        connector,
        metric_name
    ):
...
class TestAggregationProperties:
    ...
    @given(
        tract_codes=st.lists(tract_fips, min_size=2, max_size=10, unique=True)
    )
    ...
    @given(metric_values=st.lists(st.floats(min_value=0.0, max_value=1.0, allow_nan=False), min_size=2, max_size=5))
    @settings(max_examples=20, suppress_health_check=[HealthCheck.function_scoped_fixture])
