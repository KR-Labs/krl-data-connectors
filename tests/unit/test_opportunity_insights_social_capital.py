...
def mock_connector(monkeypatch, social_capital_sample, mobility_sample):
    ...
    def mock_fetch_atlas(geography="county", metrics=None, state=None, county=None, force_download=False):
...
def test_get_high_ec_areas_return_type(mock_connector):
    ...
    result = mock_connector.get_high_ec_areas(
        geography="county",
    ...
    assert all(ec_values[i] >= ec_values[i+1] for i in range(len(ec_values)-1)), \
...
def test_get_low_ec_areas_return_type(mock_connector):
    ...
    result = mock_connector.get_low_ec_areas(
        geography="county",
...
def test_compare_ec_by_state_return_type(mock_connector):
    ...
    result = mock_connector.compare_ec_by_state(
        states=["06", "36", "48"]
    )
    ...
    assert all(ec_means[i] >= ec_means[i+1] for i in range(len(ec_means)-1)), \
...
def test_get_ec_clustering_correlation_return_type(mock_connector):
    ...
    result = mock_connector.get_ec_clustering_correlation(
        geography="county"
    )
...
def test_get_social_capital_summary_return_type(mock_connector):
    ...
    result = mock_connector.get_social_capital_summary(
        geography="county",
        geo_id="06037"  # Los Angeles County
    )
...
def test_rank_areas_by_ec_return_type(mock_connector):
    ...
    result = mock_connector.rank_areas_by_ec(
        geography="county",
        top_n=3,
        ascending=False
    )
...
def test_compare_mobility_and_social_capital_return_type(mock_connector):
    ...
    assert 'clustering_county' in result.columns, "Should have social capital metric clustering_county"
