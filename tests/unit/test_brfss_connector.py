...
def test_get_health_indicators_return_type(connector):
    ...
    required_cols = [
        'year', 'state', 'state_name', 'indicator', 'prevalence',
        'sample_size', 'confidence_low', 'confidence_high',
        'stratification', 'stratification_value'
    ]
...
def test_analyze_chronic_disease_return_type(connector):
    ...
    # Validate required columns
    required_cols = [
        'geography', 'prevalence', 'diagnosed_count',
        'age_adjusted_prevalence', 'rank', 'trend_5yr'
    ]
    for col in required_cols:
        assert col in result.columns, f"Missing required column: {col}"
...
def test_get_preventive_care_return_type(connector):
    ...
    # Validate required columns
    required_cols = [
        'year', 'state', 'service_type', 'utilization_rate',
        'guideline_adherent', 'insurance_covered', 'uninsured_rate',
        'disparity_gap'
    ]
    for col in required_cols:
        assert col in result.columns, f"Missing required column: {col}"
...
def test_track_risk_behaviors_return_type(connector):
    ...
    # Validate required columns
    required_cols = [
        'year', 'behavior', 'prevalence', 'prevalence_change',
        'highest_risk_group', 'disparity_ratio'
    ]
    for col in required_cols:
        assert col in result.columns, f"Missing required column: {col}"
...
def test_analyze_health_disparities_return_type(connector):
    ...
    # Validate required columns
    required_cols = [
        'indicator', 'dimension', 'group', 'prevalence',
        'reference_group', 'reference_prevalence', 'disparity_ratio',
        'excess_cases', 'rank'
    ]
    for col in required_cols:
        assert col in result.columns, f"Missing required column: {col}"
...
def test_get_mental_health_indicators_return_type(connector):
    ...
    # Validate required columns
    required_cols = [
        'year', 'state', 'depression_prevalence', 'poor_mental_health_days',
        'frequent_mental_distress', 'anxiety_prevalence',
        'mental_health_treatment', 'unmet_need'
    ]
    for col in required_cols:
        assert col in result.columns, f"Missing required column: {col}"
...
def test_compare_states_return_type(connector):
    ...
    result = connector.compare_states(
        states=states,
    ...
    # Validate required columns
    required_cols = [
        'state', 'state_name', 'indicator', 'prevalence',
        'national_average', 'difference_from_national',
        'rank', 'percentile'
    ]
    for col in required_cols:
        assert col in result.columns, f"Missing required column: {col}"
...
def test_fetch_method_routing(connector):
    ...
    result = connector.fetch(
        query_type='preventive_care',
        service_type='flu_vaccine'
    )
    assert isinstance(result, pd.DataFrame)
    ...
    result = connector.fetch(
        query_type='disparities',
        indicator='heart_disease',
        disparity_dimension='income'
    )
    assert isinstance(result, pd.DataFrame)
