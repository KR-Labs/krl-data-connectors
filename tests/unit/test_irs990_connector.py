...
def test_search_nonprofits_return_type(irs990_connector):
    ...
    """
    nonprofits = irs990_connector.search_nonprofits(
        query='museum',
        state='NY'
    )
    ...
    expected_columns = [
        'ein', 'name', 'city', 'state', 'zip_code',
        'ntee_code', 'subsection_code', 'tax_period',
...
def test_get_by_ntee_code_return_type(irs990_connector):
    ...
    arts_orgs = irs990_connector.get_by_ntee_code(
        ntee_codes=['A'],
        state='CA'
    )
    ...
    specific_orgs = irs990_connector.get_by_ntee_code(
        ntee_codes=['A50', 'A60']
    )
    ...
    assert all(
        ntee.startswith('A5') or ntee.startswith('A6')
        for ntee in specific_orgs['ntee_code']
    )
...
def test_analyze_cultural_organizations_return_type(irs990_connector):
    ...
    museums = irs990_connector.analyze_cultural_organizations(
        state='NY',
        subsector='museums'
    )
...
def test_get_arts_nonprofits_return_type(irs990_connector):
    ...
    large_arts = irs990_connector.get_arts_nonprofits(
        state='NY',
...
def test_get_nonprofit_statistics_return_type(irs990_connector):
    ...
    """
    stats = irs990_connector.get_nonprofit_statistics(
        ntee_codes=['A'],
        group_by='state'
    )
    ...
    expected_stats = [
        ...
        'total_revenue_sum', 'total_assets_sum', 'total_expenses_sum',
        'avg_revenue', 'median_revenue', 'avg_financial_health_score'
    ]
    ...
    ntee_stats = irs990_connector.get_nonprofit_statistics(
        group_by='ntee'
    )
...
def test_fetch_method_routing(irs990_connector):
    ...
    """
    # Test search query
    search_result = irs990_connector.fetch(
        query_type='search',
        query='museum',
        state='NY'
    )
    ...
    # Test NTEE query
    ntee_result = irs990_connector.fetch(
        query_type='ntee',
        ntee_codes=['A'],
        state='CA'
    )
    ...
    orgs = irs990_connector.search_nonprofits(state='NY')
    financial_result = irs990_connector.fetch(
    ...
    # Test cultural query
    cultural_result = irs990_connector.fetch(
        query_type='cultural',
        state='CA'
    )
    ...
    # Test arts query
    arts_result = irs990_connector.fetch(
        query_type='arts',
        state='TX'
    )
