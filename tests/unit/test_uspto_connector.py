...
def test_search_patents_return_type(connector):
    ...
    required_cols = [
        'patent_id', 'title', 'abstract', 'grant_date',
        'technology_field', 'assignee_name', 'assignee_type',
        'inventor_count', 'citation_count', 'claim_count'
    ]
...
def test_analyze_innovation_clusters_return_type(connector):
    ...
    # Validate required columns
    required_cols = [
        'geography', 'geography_code', 'patent_count', 'patents_per_capita',
        'inventor_count', 'assignee_count', 'university_share',
        'avg_citation_count', 'specialization_index', 'cluster_rank'
    ]
    for col in required_cols:
        assert col in result.columns, f"Missing required column: {col}"
...
def test_track_technology_trends_return_type(connector):
    ...
    result = connector.track_technology_trends(
        technology_fields=['G06F', 'H04L', 'C12N'],
        year_start=2015,
        year_end=2024
    )
    ...
    # Validate required columns
    required_cols = [
        'year', 'technology_field', 'technology_name', 'patent_count',
        'growth_rate', 'citation_rate', 'market_share', 'trend_direction'
    ]
    for col in required_cols:
        assert col in result.columns, f"Missing required column: {col}"
...
def test_analyze_inventor_networks_return_type(connector):
    ...
    result = connector.analyze_inventor_networks(
        technology_field="G06F",
    ...
    # Validate required columns
    required_cols = [
        'inventor_name', 'inventor_id', 'patent_count', 'collaboration_count',
        'avg_team_size', 'primary_field', 'assignee_count',
        'centrality_score', 'h_index'
    ]
    for col in required_cols:
        assert col in result.columns, f"Missing required column: {col}"
...
def test_get_patent_citations_return_type(connector):
    ...
    # Validate required columns
    required_cols = [
        'patent_id', 'title', 'citation_count', 'forward_citations',
        'backward_citations', 'self_citations', 'citation_lag',
        'impact_score', 'technology_field'
    ]
    for col in required_cols:
        assert col in result.columns, f"Missing required column: {col}"
...
def test_compare_innovation_regions_return_type(connector):
    ...
    result = connector.compare_innovation_regions(
        regions=regions,
        technology_field="G06F"
    )
    ...
    # Validate required columns
    required_cols = [
        'region', 'patent_count', 'patents_per_capita', 'growth_rate',
        'university_patents', 'corporate_patents', 'avg_citation_count',
        'inventor_density', 'assignee_diversity', 'innovation_score'
    ]
    for col in required_cols:
        assert col in result.columns, f"Missing required column: {col}"
    ...
    assert ((result['university_patents'] + result['corporate_patents']) <= 
            result['patent_count'] * 1.5).all()
...
def test_get_industry_innovation_return_type(connector):
    ...
    # Validate required columns
    required_cols = [
        'year', 'industry_sector', 'technology_field', 'patent_count',
        'growth_rate', 'citation_rate', 'university_share', 'startup_share',
        'avg_claim_count', 'concentration_index'
    ]
    for col in required_cols:
        assert col in result.columns, f"Missing required column: {col}"
