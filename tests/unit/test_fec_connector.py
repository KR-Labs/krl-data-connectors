...
def test_search_candidates_return_type(connector):
    ...
    result = connector.search_candidates(office="S", state="PA", cycle=2024, limit=5)
...
def test_get_committee_finances_return_type(connector):
    ...
    result = connector.get_committee_finances(committee_type="H", cycle=2024, limit=5)
...
def test_get_contributions_return_type(connector):
    ...
    result = connector.get_contributions(
        committee_id="C00000001", min_amount=1000, cycle=2024, limit=5
    )
...
def test_analyze_fundraising_patterns_return_type(connector):
    ...
    result = connector.analyze_fundraising_patterns(committee_id="C00000001", cycle=2024)
    ...
    percentage_columns = ["small_donor_percentage", "large_donor_percentage", "pac_percentage"]
...
def test_get_expenditures_return_type(connector):
    ...
    result = connector.get_expenditures(
        committee_id="C00000001", min_amount=5000, cycle=2024, limit=5
    )
...
def test_get_campaign_statistics_return_type(connector):
    ...
    result = connector.get_campaign_statistics(office="H", cycle=2024, group_by="state")
...
def test_fetch_method_routing(connector):
    ...
    # Test candidates
    candidates = connector.fetch(query_type="candidates", office="H", limit=5)
    assert isinstance(candidates, pd.DataFrame)
    assert "candidate_id" in candidates.columns

    # Test committees
    committees = connector.fetch(query_type="committees", cycle=2024, limit=5)
    assert isinstance(committees, pd.DataFrame)
    assert "committee_id" in committees.columns

    # Test contributions
    contributions = connector.fetch(query_type="contributions", limit=5)
    assert isinstance(contributions, pd.DataFrame)
    assert "contribution_amount" in contributions.columns

    # Test expenditures
    expenditures = connector.fetch(query_type="expenditures", limit=5)
    assert isinstance(expenditures, pd.DataFrame)
    assert "disbursement_amount" in expenditures.columns

    # Test fundraising
    fundraising = connector.fetch(query_type="fundraising", committee_id="C00000001", cycle=2024)
    assert isinstance(fundraising, pd.DataFrame)
    assert "total_raised" in fundraising.columns

    # Test statistics
    statistics = connector.fetch(query_type="statistics", office="H", cycle=2024, group_by="state")
