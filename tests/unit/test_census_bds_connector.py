...
def test_get_startup_rates_return_type(connector):
    ...
    result = connector.get_startup_rates(
        year=2020,
        geography='state'
    )
    ...
    # Required columns
    required_columns = [
        'geography_name', 'geography_code', 'year',
        'establishments_total', 'establishments_births',
        'startup_rate', 'employment_births', 'avg_size_births'
    ]
...
def test_analyze_job_creation_return_type(connector):
    ...
    # Required columns
    required_columns = [
        'year', 'geography_name', 'geography_code',
        'job_creation', 'job_creation_rate',
        'job_destruction', 'job_destruction_rate',
    ...
    numeric_columns = [
        'job_creation', 'job_creation_rate',
        'job_destruction', 'job_destruction_rate',
        'net_job_creation', 'total_employment'
    ]
    ...
    assert (result['net_job_creation'] == 
            result['job_creation'] - result['job_destruction']).all()
...
def test_get_firm_age_distribution_return_type(connector):
    ...
    result = connector.get_firm_age_distribution(
        year=2020,
        geography='state'
    )
    ...
    required_columns = [
        'geography_name', 'geography_code', 'year',
        'age_0', 'age_1_to_5', 'age_6_to_10', 'age_11_plus',
        'total_firms', 'pct_young', 'avg_age'
    ]
    for col in required_columns:
        assert col in result.columns, f"Missing column: {col}"
    ...
    calculated_total = (
        result['age_0'] + result['age_1_to_5'] + 
        result['age_6_to_10'] + result['age_11_plus']
    )
...
def test_get_firm_size_distribution_return_type(connector):
    ...
    result = connector.get_firm_size_distribution(
        year=2020,
        geography='state'
    )
    ...
    required_columns = [
        'geography_name', 'geography_code', 'year',
        'size_1_to_4', 'size_5_to_9', 'size_10_to_19',
        'size_20_to_99', 'size_100_to_499', 'size_500_plus',
        'total_firms', 'pct_small'
    ]
    for col in required_columns:
        assert col in result.columns, f"Missing column: {col}"
...
def test_calculate_survival_rates_return_type(connector):
    ...
    # Required columns
    required_columns = [
        'geography_name', 'geography_code', 'cohort_year',
        'year_1_survival', 'year_2_survival', 'year_3_survival',
        'year_5_survival', 'initial_cohort_size'
    ]
    for col in required_columns:
        assert col in result.columns, f"Missing column: {col}"
    ...
    survival_columns = [
        'year_1_survival', 'year_2_survival', 
        'year_3_survival', 'year_5_survival'
    ]
...
def test_compare_economic_dynamism_return_type(connector):
    ...
    required_columns = [
        ...
        'startup_rate', 'job_creation_rate', 'firm_exit_rate',
        'net_job_creation_rate', 'pct_young_firms', 'dynamism_score'
    ]
    for col in required_columns:
        assert col in result.columns, f"Missing column: {col}"
    ...
    percentage_columns = [
        'startup_rate', 'job_creation_rate', 'firm_exit_rate',
        'net_job_creation_rate', 'pct_young_firms'
    ]
...
def test_get_sector_dynamics_return_type(connector):
    ...
    result = connector.get_sector_dynamics(
        year=2020,
        sector='51',
        geography='national'
    )
    ...
    # Required columns
    required_columns = [
        'geography_name', 'sector_code', 'sector_name',
        'establishments_total', 'establishments_births',
        'job_creation', 'startup_rate', 'avg_size'
    ]
    for col in required_columns:
        assert col in result.columns, f"Missing column: {col}"
...
def test_fetch_method_routing(connector):
    ...
    # Test firm_size
    size = connector.fetch(
        query_type='firm_size',
        year=2020,
        geography='national'
    )
