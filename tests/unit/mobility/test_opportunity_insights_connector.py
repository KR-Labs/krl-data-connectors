...
class TestInitialization:
    ...
    def test_init_default_params(self):
        ...
        assert oi._social_capital_data is None  # Kept for backward compatibility, cache dict added later
...
class TestDataFiltering:
    ...
    @patch('pandas.read_stata')
    @patch.object(OpportunityInsightsConnector, '_download_file')
    def test_filter_by_state_and_county(self, mock_download, mock_read_stata, connector, sample_stata_data):
    ...
    def test_filter_by_metrics(self, mock_download, mock_read_stata, connector, sample_stata_data):
        ...
        expected_cols = {'tract', 'county', 'state', 'cz', 'czname', 
                        'kfr_pooled_p25', 'jail_pooled_p25'}
        assert set(result.columns) == expected_cols

    @patch('pandas.read_stata')
    @patch.object(OpportunityInsightsConnector, '_download_file')
    def test_filter_nonexistent_state(self, mock_download, mock_read_stata, connector, sample_stata_data):
...
class TestErrorHandling:
        ...
        with patch.object(connector, 'session') as mock_session:
            ...
            with pytest.raises(requests.HTTPError):
                connector._download_file(
                    "https://example.com/nonexistent.dta",
                    "test.dta"
                )
    ...
    def test_fetch_without_connect(self, connector):
        ...
        with patch('pandas.read_stata') as mock_read:
            ...
            with patch.object(connector.session if connector.session else requests.Session(), 'get') as mock_get:
                mock_response = Mock()
...
class TestCaching:
    ...
    def test_download_file_caching(self, connector, tmp_path):
        ...
        with patch.object(connector, 'session') as mock_session:
            ...
            # First download
            result1 = connector._download_file(
                "https://example.com/data.dta",
                "cached_data.dta"
            )
    ...
    def test_null_values_in_metrics(self, connector, sample_stata_data):
        ...
        with patch('pandas.read_stata') as mock_read:
            ...
            with patch.object(connector, '_download_file') as mock_download:
                ...
                result = connector.fetch_opportunity_atlas(
                    geography='tract',
                    metrics=['kfr_black_p25']  # Has nulls in sample data
                )
