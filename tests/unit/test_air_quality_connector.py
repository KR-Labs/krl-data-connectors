...
class TestGetCurrentByZip:
    ...
    @patch.object(EPAAirQualityConnector, '_make_request')
    def test_get_current_by_zip_custom_distance(self, mock_request, connector, sample_current_response):
...
class TestGetForecastByZip:
    ...
    @patch.object(EPAAirQualityConnector, '_make_request')
    def test_get_forecast_by_zip_with_date_string(self, mock_request, connector, sample_forecast_response):
    ...
    @patch.object(EPAAirQualityConnector, '_make_request')
    def test_get_forecast_by_zip_with_datetime(self, mock_request, connector, sample_forecast_response):
...
class TestGetForecastByLatLon:
    ...
    @patch.object(EPAAirQualityConnector, '_make_request')
    def test_get_forecast_by_latlon_success(self, mock_request, connector, sample_forecast_response):
...
class TestGetHistoricalByZip:
    ...
    @patch.object(EPAAirQualityConnector, '_make_request')
    def test_get_historical_by_zip_with_datetime(self, mock_request, connector, sample_current_response):
    ...
    @patch.object(EPAAirQualityConnector, '_make_request')
    def test_get_historical_by_zip_with_end_date(self, mock_request, connector, sample_current_response):
        """Test historical data with date range."""
        mock_request.return_value = sample_current_response

        connector.get_historical_by_zip(
            "94102",
...
class TestGetHistoricalByLatLon:
    ...
    @patch.object(EPAAirQualityConnector, '_make_request')
    def test_get_historical_by_latlon_success(self, mock_request, connector, sample_current_response):
        """Test successful historical data by lat/lon."""
        mock_request.return_value = sample_current_response

        result = connector.get_historical_by_latlon(
            37.7749, -122.4194,
            start_date="2025-10-01"
        )
