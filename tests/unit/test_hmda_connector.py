...
def mock_connector():
    ...
    sample_loans = pd.DataFrame({
        ...
        'census_tract': ['0601.02', '0601.02', '0602.01', '0602.01', '0603.01',
                        '3606.01', '3606.02', '3607.01', '3607.02', '3608.01'],
        'lei': ['LENDER_A', 'LENDER_A', 'LENDER_B', 'LENDER_B', 'LENDER_C',
                'LENDER_A', 'LENDER_B', 'LENDER_C', 'LENDER_C', 'LENDER_A'],
        'loan_type': [1] * 10,  # Conventional
        'loan_purpose': [1] * 10,  # Home purchase
        'loan_amount': [350000, 400000, 275000, 425000, 500000,
                       650000, 700000, 600000, 550000, 675000],
        ...
        'applicant_income': [95000, 105000, 75000, 68000, 120000,
                           135000, 88000, 142000, 128000, 95000],
        'derived_msa_md': ['31080'] * 5 + ['35620'] * 5,  # LA MSA, NY MSA
        'derived_loan_to_value_ratio': [0.80, 0.75, 0.85, 0.90, 0.70,
                                        0.78, 0.88, 0.72, 0.80, 0.85],
...
def test_load_loan_data_return_type(mock_connector):
    ...
    required_cols = [
        ...
        'loan_amount', 'action_taken', 'applicant_race_1', 
        'applicant_ethnicity_1', 'applicant_sex', 'applicant_income'
    ]
...
def test_get_loans_by_state_return_type(mock_connector):
    ...
    result = mock_connector.get_loans_by_state(
        year=2022,
        state_code='CA',
        action_taken=1
    )
...
def test_get_denial_rates_return_type(mock_connector):
    ...
    result = mock_connector.get_denial_rates(
        year=2022,
        state_code='CA',
        by_race=True
    )
    ...
    assert (result['total_applications'] == result['denied'] + result['originated']).all(), \
        "Total applications should equal denied + originated"
    
    # Check data types
    assert pd.api.types.is_numeric_dtype(result['total_applications']), "total_applications should be numeric"
...
def test_get_lending_patterns_return_type(mock_connector):
    ...
    result = mock_connector.get_lending_patterns(
        year=2022,
        state_code='CA',
        group_by='county'
    )
    ...
    # Check for required columns
    required_cols = [
        'county_code', 'total_applications', 'total_originations',
        'median_loan_amount', 'origination_rate'
    ]
    for col in required_cols:
        assert col in result.columns, f"Required column '{col}' missing"
    ...
    assert (result['total_originations'] <= result['total_applications']).all(), \
        "Originations cannot exceed applications"
    
    # Check data types
    assert pd.api.types.is_numeric_dtype(result['total_applications']), "total_applications should be numeric"
    assert pd.api.types.is_numeric_dtype(result['median_loan_amount']), "median_loan_amount should be numeric"
...
def test_analyze_redlining_indicators_return_type(mock_connector):
    ...
    # Check for required columns
    required_cols = [
        'tract_category', 'total_applications', 'origination_rate',
        'denial_rate', 'median_loan_amount'
    ]
    for col in required_cols:
        assert col in result.columns, f"Required column '{col}' missing"
    ...
    assert result['tract_category'].isin(valid_categories).all(), \
...
def test_get_lender_statistics_return_type(mock_connector):
    ...
    result = mock_connector.get_lender_statistics(
        year=2022,
        state_code='CA',
        top_n=5
    )
    ...
    # Check for required columns
    required_cols = [
        'lei', 'total_applications', 'total_originations',
        'origination_rate', 'median_loan_amount'
    ]
    for col in required_cols:
        assert col in result.columns, f"Required column '{col}' missing"
    ...
    assert all(apps[i] >= apps[i+1] for i in range(len(apps)-1)), \
