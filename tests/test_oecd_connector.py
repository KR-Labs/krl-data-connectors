...
class TestOECDConnectorDataRetrieval:
    ...
    def test_get_data_with_basic_params(self, mock_get):
        ...
        mock_response.json.return_value = {
            'data': {
                'dataSets': [{'observations': {'0:0:0:0': [100]}}]
            }
        }
        mock_response.raise_for_status = MagicMock()
        mock_get.return_value = mock_response
...
class TestOECDConnectorSecurity:
    ...
    def test_xss_in_dimensions(self, mock_get):
        ...
        result = oecd.get_data(
            dataflow="OECD.SDD.NAD,DSD_NAAG@DF_NAAG_I",
            dimensions=malicious_dimensions
        )
    ...
    def test_path_traversal_in_query_params(self, mock_get):
        ...
        result = oecd.fetch(query_params={
            'query_type': 'dataflow',
            'path': malicious_path
        })
...
class TestOECDConnectorPropertyBased:
    ...
    @given(
        dimensions=st.text(
            alphabet=st.characters(whitelist_categories=('Lu', 'Ll', 'Nd'), min_codepoint=46, max_codepoint=90),
            min_size=1,
            max_size=100
        )
    )
    ...
    @given(
        obs_count=st.integers(min_value=1, max_value=1000)
    )
    @patch('requests.Session.get')
