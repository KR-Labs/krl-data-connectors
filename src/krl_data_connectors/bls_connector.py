...
class BLSConnector(BaseConnector):
    ...
    def get_series(
        ...
        if end_year - start_year + 1 > max_years:
            raise ValueError(f"Year range too large. Maximum is {max_years} years for this API key type.")
        ...
        self.logger.info(
            f"Fetching BLS series: {series_id}, years {start_year}-{end_year}"
        )
        ...
        try:
            ...
            if 'year' in df.columns and 'period' in df.columns:
                df['date'] = pd.to_datetime(
                    df['year'] + '-' + df['period'].str.replace('M', '').str.replace('Q0', 'Q').str.replace('A01', '01'),
                    errors='coerce'
                )
    ...
    def get_multiple_series(
        ...
        if len(series_ids) > max_series:
            raise ValueError(f"Too many series requested. Maximum is {max_series} for this API key type.")
