...
class FCCBroadbandConnector(BaseConnector):
    """
    ...
    def get_coverage_by_state(
        ...
        min_download_mbps: Optional[int] = None
    ) -> pd.DataFrame:
        """
        ...
        if self._coverage_data is None:
            raise ValueError(
                "No coverage data loaded. Call load_coverage_data() first."
            )
    ...
    def get_underserved_areas(
        self,
        min_download_mbps: int = 25,
        min_upload_mbps: int = 3,
        ...
        underserved = df[
            (df['max_down'] < min_download_mbps) |
            (df['max_up'] < min_upload_mbps)
        ]
    ...
    def get_speed_tier_distribution(
        self,
        state: Optional[str] = None
    ) -> Dict[str, int]:
        """
    ...
    def get_technology_availability(
        self,
        state: Optional[str] = None
    ) -> pd.DataFrame:
        """
