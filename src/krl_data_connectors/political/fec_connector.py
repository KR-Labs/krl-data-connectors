...
class FECConnector(BaseConnector):
    ...
    Example:
        ...
        >>> # Search for presidential candidates in 2024
        >>> candidates = connector.search_candidates(
        ...     office='P',
        ...     cycle=2024
        ... )
        >>> print(f"Found {len(candidates)} presidential candidates")
        >>> 
        >>> # Analyze fundraising patterns
        >>> analysis = connector.analyze_fundraising_patterns(
        ...     candidate_id='P00012345',
        ...     cycle=2024
        ... )
    """
    ...
    def search_candidates(
        ...
        candidates = pd.DataFrame({
            'candidate_id': [f'P800{i:05d}' if office == 'P' else f'H000{i:05d}' for i in range(1, 11)],
            'name': [
                'SMITH, JOHN',
                'JOHNSON, MARY',
                'WILLIAMS, ROBERT',
            ...
            'office_full': ['President' if office == 'P' else 'House' if office == 'H' else 'Senate'] * 10,
            'state': [state or 'PA'] * 10,
            'district': ['01', '02', '03', '04', '05', '06', '07', '08', '09', '10'] if office == 'H' else [None] * 10,
            'party': [party or 'DEM'] * 5 + [party or 'REP'] * 5,
            'party_full': ['Democratic'] * 5 + ['Republican'] * 5,
            'incumbent_challenge': ['I', 'C', 'O', 'C', 'I', 'C', 'O', 'C', 'I', 'C'],
            'incumbent_challenge_full': [
                'Incumbent', 'Challenger', 'Open seat', 'Challenger', 'Incumbent',
                'Challenger', 'Open seat', 'Challenger', 'Incumbent', 'Challenger'
            ],
    ...
    def get_committee_finances(
        ...
        logger.info(
            ...
            committee_id, committee_type, cycle
        )
        ...
        committees = pd.DataFrame({
            ...
            'committee_type_full': ['House' if not committee_type or committee_type == 'H' else 'Senate'] * 10,
            'designation': ['A'] * 10,  # Authorized
            'party': ['DEM'] * 5 + ['REP'] * 5,
            'cycle': [cycle or 2024] * 10,
            'receipts': [
                2_500_000, 1_800_000, 3_200_000, 1_500_000, 5_000_000,
                2_000_000, 2_800_000, 1_200_000, 3_500_000, 1_900_000
            ],
            'disbursements': [
                2_300_000, 1_700_000, 3_000_000, 1_400_000, 4_800_000,
                1_900_000, 2_700_000, 1_100_000, 3_300_000, 1_800_000
            ],
            'cash_on_hand_end_period': [
                200_000, 100_000, 200_000, 100_000, 200_000,
                100_000, 100_000, 100_000, 200_000, 100_000
            ],
            'debts_owed': [
                50_000, 25_000, 100_000, 30_000, 150_000,
                40_000, 75_000, 20_000, 120_000, 35_000
            ],
            'individual_contributions': [
                1_800_000, 1_200_000, 2_500_000, 1_000_000, 3_500_000,
                1_400_000, 2_000_000, 800_000, 2_800_000, 1_300_000
            ],
            'pac_contributions': [
                500_000, 400_000, 500_000, 300_000, 1_000_000,
                400_000, 600_000, 250_000, 500_000, 400_000
            ],
            'candidate_contributions': [
                200_000, 200_000, 200_000, 200_000, 500_000,
                200_000, 200_000, 150_000, 200_000, 200_000
            ],
            'operating_expenditures': [
                300_000, 200_000, 400_000, 180_000, 600_000,
                250_000, 350_000, 150_000, 450_000, 220_000
            ],
    ...
    def get_contributions(
        ...
        logger.info(
            ...
            committee_id, contributor_name, min_amount, cycle
        )
        ...
        contributions = pd.DataFrame({
            ...
            'contributor_employer': [
                'GOLDMAN SACHS', 'GOOGLE', 'MICROSOFT', 'EXXONMOBIL', 'COMCAST',
                'APPLE', 'GENERAL ELECTRIC', 'AMAZON', 'WELLS FARGO', 'COCA-COLA'
            ],
            'contributor_occupation': [
                'BANKER', 'ENGINEER', 'SOFTWARE DEVELOPER', 'EXECUTIVE', 'ATTORNEY',
                'ENGINEER', 'EXECUTIVE', 'SOFTWARE DEVELOPER', 'BANKER', 'MARKETING'
            ],
            'contribution_receipt_date': pd.to_datetime([
                '2024-01-15', '2024-02-20', '2024-03-10', '2024-04-05', '2024-05-12',
                '2024-06-18', '2024-07-22', '2024-08-08', '2024-09-14', '2024-10-01'
            ]),
            'contribution_amount': [
                2800, 1000, 500, 2800, 1500,
                2800, 750, 2800, 1200, 2800
            ],
        ...
        if min_amount:
            contributions = contributions[
                contributions['contribution_amount'] >= min_amount
            ]
    ...
    def analyze_fundraising_patterns(
        ...
        finances = self.get_committee_finances(
        ...
        analysis['total_contributions'] = (
            analysis['individual_contributions'] / 500
        ).astype(int)
        ...
        # Burn rate
        analysis['burn_rate'] = (
            analysis['disbursements'] / analysis['receipts'] * 100
        ).round(2)
        ...
        keep_cols = [
            'entity_id', 'entity_name', 'cycle', 'total_raised',
            'total_contributions', 'avg_contribution',
            ...
            'self_funding_amount', 'self_funding_percentage',
            'burn_rate', 'cash_on_hand_end_period'
        ]
    ...
    def get_expenditures(
        ...
        logger.info(
            ...
            committee_id, recipient_name, min_amount, cycle
        )
        ...
        expenditures = pd.DataFrame({
            ...
            'recipient_city': [
                'WASHINGTON', 'NEW YORK', 'LOS ANGELES', 'CHICAGO', 'PHILADELPHIA',
                'BOSTON', 'DENVER', 'ATLANTA', 'SEATTLE', 'MIAMI'
            ],
            'recipient_state': ['DC', 'NY', 'CA', 'IL', 'PA', 'MA', 'CO', 'GA', 'WA', 'FL'],
            'disbursement_date': pd.to_datetime([
                '2024-02-15', '2024-03-20', '2024-04-10', '2024-05-05', '2024-06-12',
                '2024-07-18', '2024-08-22', '2024-09-08', '2024-10-14', '2024-10-25'
            ]),
            'disbursement_amount': [
                50000, 25000, 35000, 15000, 40000,
                75000, 20000, 30000, 45000, 18000
            ],
            ...
            'category_code': [
                'MEDIA', 'CONSULTING', 'MEDIA', 'EVENTS', 'MAIL',
                'MEDIA', 'POLLING', 'FUNDRAISING', 'PAYROLL', 'TRAVEL'
            ],
            'cycle': [cycle or 2024] * 10,
        })
        ...
        if min_amount:
            expenditures = expenditures[
                expenditures['disbursement_amount'] >= min_amount
            ]
    ...
    def get_campaign_statistics(
        ...
        for _, candidate in candidates.iterrows():
            ...
            finance = self.get_committee_finances(
                committee_id=committee_id,
                cycle=cycle,
                limit=1
            )
        ...
        stats = all_finances.groupby(group_col).agg(
    ...
    def fetch(
        ...
        **kwargs
        ...
        """
        Main entry point for fetching FEC data.
        
        Args:
            query_type: Type of query to perform:
                - 'candidates': Search candidates
                - 'committees': Get committee finances
                - 'contributions': Get contributions
                - 'expenditures': Get expenditures
                - 'fundraising': Analyze fundraising patterns
        ...
        Example:
            >>> connector = FECConnector(api_key='YOUR_KEY')
            >>> 
            >>> # Search candidates
        ...
        if query_type == 'candidates':
            return self.search_candidates(**kwargs)
        
        elif query_type == 'committees':
            return self.get_committee_finances(**kwargs)
        
        elif query_type == 'contributions':
            return self.get_contributions(**kwargs)
        
        elif query_type == 'expenditures':
            return self.get_expenditures(**kwargs)
        
        elif query_type == 'fundraising':
            return self.analyze_fundraising_patterns(**kwargs)
        
        elif query_type == 'statistics':
            return self.get_campaign_statistics(**kwargs)
        
        else:
            raise ValueError(
                f"Unknown query_type: {query_type}. "
                "Must be one of: candidates, committees, contributions, "
                "expenditures, fundraising, statistics"
            )
