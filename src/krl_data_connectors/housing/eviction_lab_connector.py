...
class EvictionLabConnector(BaseConnector):
    """
    ...
    def get_eviction_by_geography(
        ...
        if level == 'tract':
            if self._tract_data is None:
                raise ValueError(
                    "No tract data loaded. Call load_tract_data() first."
                )
            df = self._tract_data[self._tract_data['GEOID'] == geoid]
        elif level == 'county':
            if self._county_data is None:
                raise ValueError(
                    "No county data loaded. Call load_county_data() first."
                )
    ...
    def get_eviction_trends(
        ...
        return df[['year', 'evictions', 'eviction-filings', 
