...
class IRS990Connector(BaseConnector):
    """
    ...
    def _get_api_key(self) -> Optional[str]:
        """
        Get API key for ProPublica API (optional).
    ...
    def search_nonprofits(
        ...
        """
        logger.info(
            "Searching nonprofits: query=%s, state=%s, ntee=%s",
            query, state, ntee_code
        )
        ...
        nonprofits = pd.DataFrame({
            ...
            'zip_code': ['10021', '10018', '10019', '10023', '10019', 
                         '11217', '10019', '10023', '10019', '10027'],
            'ntee_code': ['A50', 'A51', 'A60', 'A60', 'A50',
                          'A60', 'A60', 'A60', 'A60', 'A60'],
            'subsection_code': ['501(c)(3)'] * 10,
            'tax_period': [202212] * 10,
            'total_revenue': [300_000_000, 250_000_000, 80_000_000, 150_000_000, 200_000_000,
                             40_000_000, 35_000_000, 45_000_000, 30_000_000, 25_000_000],
            'total_assets': [5_000_000_000, 1_500_000_000, 500_000_000, 800_000_000, 3_000_000_000,
                            200_000_000, 150_000_000, 250_000_000, 120_000_000, 100_000_000],
            'total_expenses': [280_000_000, 240_000_000, 75_000_000, 145_000_000, 190_000_000,
                              38_000_000, 33_000_000, 43_000_000, 28_000_000, 24_000_000],
            'program_expenses': [220_000_000, 200_000_000, 60_000_000, 120_000_000, 150_000_000,
                                30_000_000, 28_000_000, 35_000_000, 23_000_000, 20_000_000],
            'administrative_expenses': [40_000_000, 30_000_000, 10_000_000, 18_000_000, 30_000_000,
                                       5_000_000, 3_000_000, 5_000_000, 3_000_000, 2_500_000],
            'fundraising_expenses': [20_000_000, 10_000_000, 5_000_000, 7_000_000, 10_000_000,
                                    3_000_000, 2_000_000, 3_000_000, 2_000_000, 1_500_000],
            'grants_made': [50_000_000, 20_000_000, 5_000_000, 10_000_000, 30_000_000,
                           2_000_000, 1_000_000, 2_000_000, 1_000_000, 500_000],
        })
        ...
        if ntee_code:
            ...
            nonprofits = nonprofits[
                nonprofits['ntee_code'].str[0].isin(ntee_majors)
            ]
        
        # Apply query filter if specified (simple name matching)
        if query:
            nonprofits = nonprofits[
                nonprofits['name'].str.contains(query, case=False, na=False)
            ]
    ...
    def get_by_ntee_code(
        ...
        return self.search_nonprofits(
            state=state,
            ntee_code=ntee_codes,
            limit=10000
        )
    ...
    def get_financial_metrics(
        ...
        organizations: Optional[pd.DataFrame] = None
    ) -> pd.DataFrame:
        """
        ...
        metrics['operating_reserve_months'] = (
            net_assets / (metrics['total_expenses'] / 12)
        ).round(1)
    ...
    def analyze_cultural_organizations(
        ...
        Example:
            ...
            ... )
        """
        logger.info("Analyzing cultural organizations: state=%s, subsector=%s",
                   state, subsector)
    ...
    def get_arts_nonprofits(
        ...
        Example:
            ...
            ... )
        """
        logger.info("Fetching arts nonprofits: state=%s, min_revenue=%s",
                   state, min_revenue)
    ...
    def get_nonprofit_statistics(
        ...
        Example:
            ...
            ... )
        """
        logger.info("Calculating nonprofit statistics: ntee=%s, state=%s, group_by=%s",
                   ntee_codes, state, group_by)
        ...
        stats = orgs_with_metrics.groupby(group_col).agg(
